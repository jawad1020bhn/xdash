/* =============================================================================
   card v3 — one media tile, three shapes (grid / rail / hero).
   ...
   In selection mode the tap target flips to the checkbox, which is the one
...
export function tile(item, list, { shape = "grid", eager = false, index } = {}) {
  const p = post(item);
  const idx = index ?? (Array.isArray(list) ? list.indexOf(item) : 0);

  const el = h(`button.tile tile--${shape}`, {
    type: "button",
  ...
  const media = mediaBox(item, p, { eager, sizes: shape === "hero" ? "(max-width: 719px) 92vw, 720px" : undefined });
  media.append(veil);
...
function itemById(id) {
  return state.index.media.find((m) => m.id === id) || null;
}
...
/** A horizontal snap rail. Cards are real DOM nodes; images stay lazy. */
export function rail(items, { label, eager = 2 } = {}) {
  return h("div.rail__track", { role: "list", "aria-label": label },
    items.map((item, i) => h("div", { role: "listitem" }, tile(item, items, { shape: "rail", eager: i < eager, index: i }))),
  );
}
