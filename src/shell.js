...
async function openMenu() {
  ...
  const rows = [
    { icon: "spark", hue: "var(--hue-a)", label: "Surprise me", hint: "Open a random item from the archive", run: surprise },
    { icon: "star", hue: "var(--hue-e)", label: "Starred", hint: `${stats().starred} items saved with a star`, run: () => {
      sheet.close();
      set({ route: "library" });
    ...
    { icon: document.documentElement.dataset.theme === "dark" ? "sun" : "moon", hue: "var(--hue-d)",
      ...
      hint: `Currently following ${state.prefs.themeMode}`, run: () => { sheet.close(); toggleTheme(); } },
    { icon: "eyeOff", hue: "var(--hue-c)", label: state.prefs.blurMedia ? "Media is blurred" : "Blur media", hint: "Privacy blur until you tap a tile",
    ...
    { icon: "keyboard", hue: "var(--hue-c)", label: "Keyboard shortcuts", hint: "Everything the keys do", run: () => { sheet.close(); showShortcuts(); } },
  ];
